  ...
  if (QS == true){                       // Quantified Self flag is true when arduino finds a heartbeat
	...
	QS = false;                      // reset the Quantified Self flag for next time  
        if (bluetoothConnected == true) {
          String sendingString = String(IBI);
          int sendingStringLength = sendingString.length();
          uint8_t sendBuffer[10];
          sendingString.getBytes(sendBuffer, 10);
          uint8_t length=sendingStringLength;
          if (uart_tx(sendBuffer, length)) {
